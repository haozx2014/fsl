...
public:
  ...
  void Update(const Ar1cParameters& dist, int nTimes);
...
class Ar1cNoiseModel : public NoiseModel {
 public:
  ...
  virtual void Precalculate( const ColumnVector& sampleData );
  ...
  { UpdateAlpha(theta, linear, data);
    UpdatePhi(theta, linear, data); }
...
public:
  ...
  // Constructor/destructor
  Ar1cParameters(int nAlpha, int nPhi) : 
        alpha(nAlpha), phis(nPhi) { return; }
  Ar1cParameters(const Ar1cParameters& from) : 
        alpha(from.alpha), phis(from.phis) { return; }
  virtual ~Ar1cParameters() { return; }
