...
{
  Tracer_Plus tr("MVNDist::Load");
  // Load the means:

  if (filename == "hardcodedsimple")
    {
      throw Invalid_option("hardcodedsimple is a very bad idea");
      //SimpleFwdModelIdStruct::LoadHardcodedInitialMeans(means);
      assert(means.Nrows() == len);
      // alternative solution: SetSize(means.Nrows());
    }
  else if (filename == "hardcodedzeros")
    {
      assert(len > 0); // we need to know this
      means = 0;
    }
  else
    throw Invalid_option("Only 'modeldefault' and 'hardcodedzero' are accepted in MVNDist::Load");
     
  SetPrecisions(IdentityMatrix(len) * 1e-12);
