...
class NoiseModel {
 public:
  ...
  // Miscellaneous functions every Noise Model needs to implement:
  ...
  virtual void DumpPrior(const string indent = "") const = 0;
  virtual void DumpPosterior(const string indent = "") const = 0;
 ...
 private:
  ...
  const NoiseModel& operator=(const NoiseModel&)
    { assert(false); }
