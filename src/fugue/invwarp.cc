...
Option<string> refvolname(string("-r,--ref"), string(""),
		       string("filename for reference image (determines inverse warpvol's FOV and pixdims)"),
		       false, requires_argument);
...
{
  // fill each 3D image in invol with values generated by successive
  //  mean dilation (starting with the mask and growing until all voxels
  //  have been filled)
  ...
  while (fillmask.sum()<(fillmask.nvoxels()-0.5)) {
    ...
    if (verbose.value()) {
      cout << " Filled " << fillmask.sum() << " voxels out of " << fillmask.nvoxels() << endl;
    }
  }
