# FidFolderDlg.tcl --
#
#	Implements the FidFolder Selection Dialog widget.
#
...
tixWidgetClass tixFidFolderSelectDialog {
    -classname TixFidFolderSelectDialog
...
proc tixFidFolderSelectDialog:ConstructTopFrame {w frame} {
    upvar #0 $w data

    tixChainMethod $w ConstructTopFrame $frame

    set data(w:fsbox) [tixFidFolderSelectBox $frame.fsbox \
	-command "tixFidFolderSelectDialog:Invoke $w"]
    pack $data(w:fsbox) -expand yes -fill both
}

proc tixFidFolderSelectDialog:SetBindings {w} {
    upvar #0 $w data
...
proc tixFidFolderSelectDialog:Invoke {w filename} {
    upvar #0 $w data
