# FidFolderInfoDlg.tcl --
#
#	Implements the FidFolderInfo Selection Dialog widget.
#
...
tixWidgetClass tixFidFolderInfoSelectDialog {
    -classname TixFidFolderInfoSelectDialog
...
proc tixFidFolderInfoSelectDialog:ConstructTopFrame {w frame} {
    upvar #0 $w data

    tixChainMethod $w ConstructTopFrame $frame

    set data(w:fsbox) [tixFidFolderInfoSelectBox $frame.fsbox \
	-command "tixFidFolderInfoSelectDialog:Invoke $w"]
    pack $data(w:fsbox) -expand yes -fill both
}

proc tixFidFolderInfoSelectDialog:SetBindings {w} {
    upvar #0 $w data
...
proc tixFidFolderInfoSelectDialog:Invoke {w filename} {
    upvar #0 $w data
