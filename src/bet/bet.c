...
FDT lmin, lmax;
int k, l;
double d, nx=v[i].nx, ny=v[i].ny, nz=v[i].nz, sx, sy, sz, fit, sn, stx, sty, stz;
...

{
  ...
  double lthresh, local_brain_threshold=brain_threshold, dscale=MIN(scale,1);
...
/* }}} */
  ...
  x=FTOI((v[i].x-d*nx)/im.xv); y=FTOI((v[i].y-d*ny)/im.yv); z=FTOI((v[i].z-d*nz)/im.zv);
  ...
  if (all_inside_image)
    {
      for(d=2*dscale;d<COST_SEARCH;d+=dscale)
	{
	  x=FTOI((v[i].x-d*nx)/im.xv); y=FTOI((v[i].y-d*ny)/im.yv); z=FTOI((v[i].z-d*nz)/im.zv);
	  lnew=IA(in,x,y,z);
	  lmin = MIN(lmin,lnew);
