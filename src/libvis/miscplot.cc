...
void miscplot::timeseries(const Matrix& mat, string filename, string title, 
  ...
  for(int ctr1=1;ctr1<=numlines; ctr1++)
    ...
    for(int ctr1=1;ctr1<=numlines; ctr1++)
      for(int ctr2=1;ctr2<=numpoint; ctr2++)
        if ( (ctr2==1 /*|| ctr2==numpoint*/)  && tr<0) 
 	 {
 	   data[(ctr1-1)*numpoint + ctr2-1] = GDC_NOVALUE;
 	 }
   if (tr<0) tr=-tr;
  ...
  //if log10(range) > 0 then no dp needed
  // else number needed = -(int)log10range +1
  // create a (C)string then pass to GDC
  if (range<1)   GDC_ylabel_density = 70;
  else   GDC_ylabel_density = 50;
  if (range>10) GDC_ylabel_fmt = "%.0f";
...
void miscplot::boxplot(const ColumnVector& vec, string filename, string title){
  ...
  for(int i=1;i<=data.Nrows();i++){
    GDC_scatter[scat_ctr].point = data(i,1);
    GDC_scatter[scat_ctr].val   = data(i,2);
...
void miscplot::boxplot(string filename, string title){
  ...
  int scatter_points = (60 + 40 + 40 + 500 + bp_outlierindex.size()) * num_points;
  if(bp_notched)
    scatter_points += 120 * num_points;
  ...
  for( int i = 1; i < num_points; i++ ){
    for ( int k = 1; k <= 60; k++ ){
      scat[scat_ctr].point = float(i)-0.3+0.01*k;
  ...
  GDC_scatter         = scat;
  GDC_num_scatter_pts = scatter_points;
